...
export default function ResultViewer() {
  ...
  useEffect(() => {
    ...
    setRequirementHistory(initialSelectedRequirement ? [initialSelectedRequirement] : []);
    setGraphResponses([]);
    setGraphError("");
  ...
  const handleGoBack = () => {
    if (requirementHistory.length > 1) {
      const newHistory = [...requirementHistory];
      newHistory.pop(); // Remove current requirement
      const previousRequirement = newHistory[newHistory.length - 1];
      setRequirementHistory(newHistory);
      setSelectedRequirement(previousRequirement);
    } else {
  ...
  const selectedGraphForFullscreen = graphResponses?.[detailGraphIndex]?.generated_code;
